...
class _HistoryDetailScreenState extends State<HistoryDetailScreen> {
  final _fmtCur  = NumberFormat.currency(locale: 'en_US', symbol: '\$', decimalDigits: 2);
  ...
  List<({String label, String value})> _inputRows(dynamic s) => [
    ...
    (label: s.interestRate,   value: '${_d('interest_rate').toStringAsFixed(2)}%'),
    (label: s.monthlyPayment, value: _fmtCur.format(_d('monthly_payment'))),
    if (_d('extra_payment') > 0)
      (label: s.extraPayment, value: '${_fmtCur.format(_d('extra_payment'))}/mo'),
  ];
  ...
  List<({String label, String value})> _resultRows(dynamic s) {
    ...
    return [
      ...
      (label: s.interestLabel,  value: _fmtCur.format(_d('interest_rate') > 0
  ...
  Future<void> _exportPdf(BuildContext context, dynamic s) async {
    ...
    pdf.addPage(pw.Page(
      ...
      build: (_) => pw.Column(
        ...
        children: [
          pw.Text('Loan Payoff US — ${s.historyDetail}',
              style: pw.TextStyle(fontSize: 20, fontWeight: pw.FontWeight.bold)),
          if (ts != null)
            pw.Text(DateFormat('MMM d, yyyy').format(ts),
          ...
          pw.Text(s.results,
              style: pw.TextStyle(fontSize: 13, fontWeight: pw.FontWeight.bold)),
          pw.SizedBox(height: 6),
          ..._resultRows(s).map((r) => _pdfRow(r.label, r.value, highlight: true)),
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                ..._inputRows(s).map((r) =>
                                    _detailRow(context, r.label, r.value)),
                              ],
                            ),
                          ),
                        ),
