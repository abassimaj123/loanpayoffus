...
class LoanCalculator {
  static double computeMonthlyPayment(
      double amount, double annualRatePct, int termMonths) {
    if (annualRatePct == 0) return amount / termMonths;
    final r = annualRatePct / 100 / 12;
    return amount * (r * math.pow(1 + r, termMonths)) /
  ...
  static List<AmortizationEntry> buildSchedule(
    ...
    while (balance > 0.01 && month <= 600) {
      ...
      schedule.add(AmortizationEntry(
        ...
        interest: interest, balance: balance, extraPayment: extraPayment,
      ));
      month++;
      if (balance <= 0) break;
    }
  ...
  static PayoffResult calculate(LoanInput input) {
    final normalSched = buildSchedule(
      input.loanAmount, input.interestRatePct, input.monthlyPayment, 0);
    final extraSched  = buildSchedule(
      input.loanAmount, input.interestRatePct, input.monthlyPayment,
      input.extraPayment);
    ...
    final interestNormal = normalSched.fold<double>(0, (s, e) => s + e.interest);
    final interestExtra  = extraSched.fold<double>(0,  (s, e) => s + e.interest);
    final interestSaved  = (interestNormal - interestExtra).clamp(0.0, double.infinity);
