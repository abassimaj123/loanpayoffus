...
const _fontPath = r'C:\Users\DALI\AppData\Local\Pub\Cache\hosted\pub.dev\provider-6.1.5+1\extension\devtools\build\assets\fonts\MaterialIcons-Regular.otf';
...
Future<Uint8List> _capture(WidgetTester tester, IconData icon, List<Color> colors, double size) async {
  final key = GlobalKey();
  final r = size * 0.22;
  ...
  group('RentBuyUS icons', () {
    for (final (density, size) in _densities) {
      testWidgets(density, (t) async {
        ...
        final bytes = await _capture(t, Icons.home_work_rounded, rbColors, size.toDouble());
  ...
  group('LoanPayoffUS icons', () {
    for (final (density, size) in _densities) {
      testWidgets(density, (t) async {
        ...
        t.view.devicePixelRatio = 1.0;
        addTearDown(t.view.resetPhysicalSize);
        final bytes = await _capture(t, Icons.monetization_on_rounded, lpColors, size.toDouble());
    ...
    testWidgets('splash 512', (t) async {
      ...
      t.view.devicePixelRatio = 1.0;
      addTearDown(t.view.resetPhysicalSize);
      final bytes = await _capture(t, Icons.monetization_on_rounded, lpColors, 512);
